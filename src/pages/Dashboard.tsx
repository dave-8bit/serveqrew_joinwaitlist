...
// Notice 'supabase' is removed from imports below
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-3 sm:p-4 pt-24 sm:pt-32 pb-20">
      <div className="max-w-5xl mx-auto space-y-6 sm:space-y-8">
        ...
        {/* Header: Focused purely on Greeting now */}
        <motion.div 
        ...
        >
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-2">
               ...
               <span className="text-[8px] font-black uppercase tracking-[0.3em] text-secondary-teal">Active Session</span>
            </div>
          ...
          {/* Note: Sign out is now in the Navigation bar at the top! */}
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          ...
          >
            ...
            <p className="text-5xl sm:text-6xl font-black italic mt-3 sm:mt-4 text-white group-hover:text-secondary-teal transition-colors">{data.profile.referrals}</p>
            <p className="text-secondary-teal font-bold text-[10px] sm:text-xs mt-2 italic">Total Referrals Secured</p>
          </motion.div>
          ...
          >
            <div className="min-w-0">
              <h4 className="text-white font-black uppercase italic text-[10px] sm:text-xs tracking-widest mb-3 sm:mb-4">Your Deployment Link</h4>
              <div className="bg-black/40 p-3 rounded-xl border border-white/5 font-mono text-[11px] sm:text-sm text-secondary-teal truncate mb-4 select-all">
                {data.profile.shareLink}
              </div>
            </div>
            <button 
              onClick={copyToClipboard}
              className={`w-full py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-black uppercase italic flex items-center justify-center gap-2 text-xs sm:text-base transition-all active:scale-[0.98] ${
                copied ? 'bg-green-600' : 'bg-secondary-teal hover:opacity-90 shadow-[0_0_20px_rgba(0,128,128,0.3)]'
              }`}
            >
              {copied ? <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5" /> : <Share2 className="w-4 h-4 sm:w-5 sm:h-5" />}
              {copied ? 'Link Secured' : 'Share Link'}
            </button>
          </motion.div>
        </div>
        ...
        >
          <div className="p-6 sm:p-8 border-b border-white/5 text-lg sm:text-xl font-black uppercase italic flex items-center gap-3 bg-white/[0.01]">
             <Users className="text-secondary-teal w-5 h-5 sm:w-6 sm:h-6" /> Recent Operatives
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  <th className="px-5 sm:px-8 py-4 text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-slate-500 text-right">Joined</th>
                </tr>
              </thead>
              <tbody>
                {data.recentReferrals.length > 0 ? (
                  data.recentReferrals.map((ref, i) => (
                    <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                      <td className="px-5 sm:px-8 py-4">
                        <div className="font-bold text-xs sm:text-sm group-hover:text-secondary-teal transition-colors">{ref.name}</div>
                      ...
                      <td className="px-5 sm:px-8 py-4 text-slate-500 text-[10px] sm:text-xs text-right">
                  ...
                  <tr>
                    <td colSpan={3} className="px-8 py-12 text-center text-slate-600 italic text-xs sm:text-base">
                      No operatives found. Use your link to recruit and rise the ranks.
                    </td>
                  </tr>
                )}
              </tbody>
